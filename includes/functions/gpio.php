...
/*----------------------------------------------------------------------------
  Saves the web-configurable GPIO settings
----------------------------------------------------------------------------*/
function gpio_save_config($pins, $pins_per_row, $update_interval)
{
  $config = "<?\n";
  $config .= "/*------------------------------------------------------------------------------\n";
  $config .= "  BerryIO GPIO Settings\n";
  $config .= "  Generated by the GPIO configuration page\n";
  $config .= "  Date: ".date('Y-m-d')." | Revision: 1\n";
  $config .= "------------------------------------------------------------------------------*/\n\n";
  $config .= "\$GLOBALS['GPIO_PINS'] = array(\n";

  foreach($pins as $pin => $name)
    $config .= "  ".(int)$pin." => ".var_export($name, TRUE).",\n";

  $config .= ");\n\n";
  $config .= "define('GPIO_PINS_PER_ROW', ".(int)$pins_per_row.");\n";
  $config .= "define('GPIO_UPDATE_INTERVAL', ".(int)$update_interval.");\n";
  $config .= "define('GPIO_SETTINGS_VERSION', '3');\n";

  return @file_put_contents(SETTINGS.'gpio.php', $config, LOCK_EX) !== FALSE;
}
